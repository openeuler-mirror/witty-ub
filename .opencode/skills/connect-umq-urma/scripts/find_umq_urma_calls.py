#!/usr/bin/env python3
"""Find physical UMQ call sites that invoke URMA public APIs."""
...
OUTPUT_FILE = Path("/tmp/umq_urma_calls.json")
...
def write_json(
    ...
    root_ids = {name: "urma_{:03d}".format(index + 1) for index, name in enumerate(roots)}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Find physical UMQ calls to URMA public APIs."
    )
...
def main() -> int:
    ...
    print("UMQ-to-URMA call sites written to {}".format(OUTPUT_FILE))
    return 0
