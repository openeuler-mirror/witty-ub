...
| 字段 | access 根 | runtime 节点 | 备注 |
| --- | --- | --- |
| `故障编号` | `kvcache_access_NNN` | `kvcache_runtime_NNN` | 如有必要增加位数 |
...
| `故障原因` | 直接取错误码信息表 | 一句话说明触发条件、失败来源和影响，建议结合日志内容，以 `故障名称` 为基础扩写 | 通用模板："执行X时，Y组件发生Z失败，导致该操作无法完成。"；不局限于代码层面或软件层面，可能的硬件根因也可以编写 |
| `解决办法` | 按错误码语义给出可能的解决方法 | 必须分析物理点、当前 callable 及必要上下游源码后给出可能的解决办法；无法唯一确定时注明适用条件并给出源码支持的排查或修复方向 | 不局限于代码层面或软件层面，可能的硬件排查方向也可以编写，不得默认填 `无` |
...
每个 runtime 节点的`解决办法`都必须阅读故障点所在源码，并按需追踪触发条件、Status 传播、调用链和资源生命周期。优先写明需修正的参数、状态、资源、配置、调用顺序或失败处理；源码只能支持候选方案时，使用“可能需要……”等有条件表述，并给出验证方向，不得臆造唯一结论。
