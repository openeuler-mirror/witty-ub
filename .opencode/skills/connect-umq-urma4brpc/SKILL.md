---
name: connect-umq-urma4brpc
description: 分析 UMQ 对 URMA 公共 API 的直接或动态函数表调用，采用 API 边界故障注入语义，把能在 UMQ 侧传播到具体错误日志的返回值、errno、输出或异步失败状态连接到 urma4brpc 公共 API 根节点，并处理 urma_ack_jfc 的明确 void 特例。用于生成、更新或复核 data/failure_mode_tree.json 中 UMQ 到 URMA4BRPC 的跨组件故障边。
---

# Connect UMQ URMA4BRPC

## 核心语义

普通边的源是 UMQ 具体日志故障节点，目标是 `urma4brpc` 公共 API 根节点：
...
- `/tmp/umq_urma4brpc_calls.json`：物理调用点；
- `/tmp/umq_urma4brpc_edge_analysis.json`：逐调用点分析记录；
- `failure_mode_tree.json.umq`：只在已有 UMQ 具体节点的 value 末尾追加确认的 `urma4brpc_xxx`。

不得修改节点定义文件、`failure_mode_tree.json.urma4brpc` 或其他顶层对象。`urma4brpc_xxx` 只作为 value 中的下游引用，
不得新增为 `umq` 的 key。
...
```bash
python3 .opencode/skills/connect-umq-urma4brpc/scripts/find_umq_urma4brpc_calls.py \
...
python3 .opencode/skills/failure-mode-generation-urma-for-brpc/scripts/generate_urma_callchains.py \
  <URMA的lib/urma源码路径>
```

检查：

- 四个输出的源码路径与本次输入一致：`umq_urma4brpc_calls.json`、`umq_log_err.json`、`umq_api_callchains.json`、`urma_api_callchains.json`；
...
按 `/tmp/urma_api_callchains.json.roots` 顺序从 `urma4brpc_001` 建立 URMA 根映射。每个目标必须：

- 同时存在于 URMA 节点定义和 `failure_mode_tree.json.urma4brpc`；
...
生成 `/tmp/umq_urma4brpc_edge_analysis.json`，至少包含：
