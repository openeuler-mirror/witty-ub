...
test('主来源为空时兜底：[timing] 报告取最新一条', () => {
  ...
  for (const fromFile of [null, undefined]) {
...
test('JSON 损坏或形状不对 → null，不抛出', () => {
  ...
  const partial = resolveParseTimingReport(
    { total_s: 3, stages: [null, { label: '无 stage' }] },
    [],
  )
  assert.ok(partial)
...
test('占比重演后端样例口径：各阶段占比之和不超过 100%', () => {
  const report = resolveParseTimingReport({ ...timingObject, end_to_end_s: 16.4 }, [])
  assert.ok(report)
  const sum = report.stages.reduce(
    (acc, stage) =>
      acc + Number(formatParseTimingShare(stage.wall_s, report.total_s).replace('%', '')),
    0,
  )
