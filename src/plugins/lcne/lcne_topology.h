...
public:
    ...
    LcneResult GetHisTopolgy(std::vector<std::shared_ptr<topology::node::Node>> &nodes,
                             std::vector<std::shared_ptr<topology::node::UbController>> &ubcs,
                             std::vector<std::shared_ptr<topology::node::Port>> &ports);
    LcneResult GetCurTopolgy(std::vector<std::shared_ptr<topology::node::Node>> &nodes,
                             std::vector<std::shared_ptr<topology::node::UbController>> &ubcs,
                             std::vector<std::shared_ptr<topology::node::Port>> &ports);
    LcneResult RegLinkNotifyHttpHandler();
    rack::com::RackComResult<rack::com::RackHttpResponse> LinkNotifyHandlerFunc(const rack::com::RackComContext &ctx,
                                                                                const rack::com::RackHttpRequest &req);
    rack::com::RackComResult<rack::com::RackHttpResponse> GetLcneTopologyInitHandlerFunc(
        const rack::com::RackComContext &ctx, const rack::com::RackHttpRequest &req);

    rack::com::RackComResult<rack::com::RackHttpResponse> GetLcneTopologyHandlerFunc(
        const rack::com::RackComContext &ctx, const rack::com::RackHttpRequest &req);

private:
    std::shared_ptr<lcne::collector::LcneNodeCollector> lcneCollector;
    std::shared_ptr<topology::node::NodeLocalCollectorModule> nodeLocalCollectorModule;
