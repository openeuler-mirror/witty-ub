                                ...
                                std::unordered_map<uint32_t, XmlPhysicalPort> &xml_physical_ports)
{
    ...
    if (lcne::common::convertTextToUint<uint32_t>(physical_port_id_ele, physicalPortId) == LCNE_FAIL) {
    ...
    std::string physicalPortStatus = physical_port_status_ele->GetText();
    ...
    xml_physical_ports[physicalPortId] = XmlPhysicalPort(physicalPortId, physicalPortStatus, remote_slot,
...
LcneResult ParseXmlNode(tinyxml2::XMLElement *node_element, std::map<LcneKey, XmlNode> &xml_nodes)
{
    ...
    std::string ubpuType = ubpu_type_ele->GetText();
    LcneKey node_key = std::make_tuple(slot, ubpu, iou);
    xml_nodes[node_key] = XmlNode(slot, ubpu, iou, ubpuType, physical_ports);
    return LCNE_SUCCESS;
}
...
LcneResult ParseXmlIouInfo(tinyxml2::XMLElement *iou_info_element, std::map<LcneKey, XmlIouInfo> &xml_iou_infos)
{
    ...
    std::string busControllerEid = bus_controller_eid_ele->GetText();
    uint32_t slotId;
    uint32_t ubpuId;
    uint32_t iouId;
    if (lcne::common::convertTextToUint<uint32_t>(slot_id_ele, slotId) == LCNE_FAIL) {
        LOG_ERROR << "getXMLIouInfos-Error: convert slot id to uint32 failed";
        return ret;
    }
    if (lcne::common::convertTextToUint<uint32_t>(ubpu_id_ele, ubpuId) == LCNE_FAIL) {
        LOG_ERROR << "getXMLIouInfos-Error: convert ubpu id to uint32 failed";
        return ret;
    }
    if (lcne::common::convertTextToUint<uint32_t>(iou_id_ele, iouId) == LCNE_FAIL) {
    ...
    std::string primaryCna = primary_cna_ele->GetText();
    if (lcne::common::checkXML(iou_status_ele) == LCNE_FAIL) {
        LOG_ERROR << "getXMLIouInfos-Error: check iou status element failed";
        return ret;
    }
    std::string iouStatus = iou_status_ele->GetText();
    LcneKey iou_info_key = std::make_tuple(slotId, ubpuId, iouId);
    xml_iou_infos[iou_info_key] =
        XmlIouInfo(guid, busControllerEid, slotId, ubpuId, iouId, primaryCna, iouStatus);
    return LCNE_SUCCESS;
}
...
LcneResult ParseXmlAddress(tinyxml2::XMLElement *address_element, std::map<LcneKey, XmlAddress> &xml_addresses)
{
    ...
    std::string busPrimaryCna = bus_primary_cna_ele->GetText();
    ...
    LcneKey address_key = std::make_tuple(slotId, ubpuId, iouId);
    xml_addresses[address_key] = XmlAddress(slotId, ubpuId, iouId, busPrimaryCna, xml_physical_ports);
    return LCNE_SUCCESS;
}
} // namespace
...
LcneResult getXMLNodes(std::string requestPath, std::map<LcneKey, XmlNode> &xml_nodes)
{
    LcneResult ret = LCNE_FAIL;
    std::string response;
    ret = lcne::common::GetHttpData(response, requestPath);
    if (ret != LCNE_SUCCESS) {
    ...
    if (result != tinyxml2::XML_SUCCESS) {
        LOG_ERROR << "getXMLNodes-Error: parse XML failed, error: " << doc.ErrorName();
        return ret;
    }
    ...
    tinyxml2::XMLElement *nodes_element = root_element->FirstChildElement("nodes");
    if (!nodes_element) {
...
LcneResult getXMLIouInfo(std::string requestPath, std::map<LcneKey, XmlIouInfo> &xml_iou_infos)
{
    ...
    tinyxml2::XMLError result = doc.Parse(response.c_str());
    if (result != tinyxml2::XML_SUCCESS) {
        LOG_ERROR << "getXMLIouInfos-Error: parse XML failed, error: " << doc.ErrorName();
        return ret;
    }
    tinyxml2::XMLElement *root_element = doc.RootElement();
    if (!root_element) {
    ...
    tinyxml2::XMLElement *iou_infos_element = root_element->FirstChildElement("iou-infos");
...
LcneResult getXMLAddress(std::string requestPath, std::map<LcneKey, XmlAddress> &xml_addresses)
{
    ...
    tinyxml2::XMLError result = doc.Parse(response.c_str());
    if (result != tinyxml2::XML_SUCCESS) {
        LOG_ERROR << "getXMLAddress-Error: parse XML failed, error: " << doc.ErrorName();
        return ret;
    }
    tinyxml2::XMLElement *root_element = doc.RootElement();
    if (!root_element) {
    ...
    tinyxml2::XMLElement *addresses_element = root_element->FirstChildElement("addresses");
                                   ...
                                   std::unordered_map<uint32_t, XmlAddressPhysicalPort> &xml_physical_ports)
{
    LcneResult ret = LCNE_FAIL;
    if (!element) {
        LOG_ERROR << "getAddressPhysicalPorts-Error: physical ports element is null";
        return ret;
    }
    for (tinyxml2::XMLElement *physical_port_element = element->FirstChildElement("physical-port");
         physical_port_element != nullptr;
         physical_port_element = physical_port_element->NextSiblingElement("physical-port")) {
        tinyxml2::XMLElement *physical_port_id_ele = physical_port_element->FirstChildElement("physical-port-id");
        tinyxml2::XMLElement *port_cna_ele = physical_port_element->FirstChildElement("port-cna");
        tinyxml2::XMLElement *bus_port_cna_ele = physical_port_element->FirstChildElement("bus-port-cna");
        uint32_t physicalPortId;
        if (lcne::common::convertTextToUint<uint32_t>(physical_port_id_ele, physicalPortId) == LCNE_FAIL) {
            LOG_ERROR << "getAddressPhysicalPorts-Error: convert port num to uint32 failed";
            return ret;
        }
        if (lcne::common::checkXML(port_cna_ele) == LCNE_FAIL) {
            LOG_ERROR << "getAddressPhysicalPorts-Error: check port cna element failed";
            return ret;
        }
        std::string port_cna = port_cna_ele->GetText();
        if (lcne::common::checkXML(bus_port_cna_ele) == LCNE_FAIL) {
            LOG_ERROR << "getAddressPhysicalPorts-Error: check bus port cna element failed";
            return ret;
        }
        std::string bus_port_cna = bus_port_cna_ele->GetText();
        xml_physical_ports[physicalPortId] = XmlAddressPhysicalPort(physicalPortId, port_cna, bus_port_cna);
    }
    ret = LCNE_SUCCESS;
    return ret;
}
...
LcneResult GetXmlLogicEntities(std::string requestPath, std::shared_ptr<XmlLogicEntity> &xmlLogicEntity)
{
    ...
    ret = lcne::common::GetHttpData(response, requestPath);
    if (ret != LCNE_SUCCESS) {
        LOG_ERROR << "GetXmlLogicEntities-Error: getHttpData failed, result: " << ret;
        return ret;
    }
    tinyxml2::XMLDocument doc;
    tinyxml2::XMLError result = doc.Parse(response.c_str());
    if (result != tinyxml2::XML_SUCCESS) {
        LOG_ERROR << "GetXmlLogicEntities-Error: parse XML failed, error: " << doc.ErrorName();
        return ret;
    }
    tinyxml2::XMLElement *root_element = doc.RootElement();
    if (!root_element) {
    ...
    tinyxml2::XMLElement *state_ele = logic_entity_element->FirstChildElement("state");
    ...
    xmlLogicEntity = std::make_shared<XmlLogicEntity>(state);
    ret = LCNE_SUCCESS;
    return ret;
}
                             ...
                             std::vector<std::shared_ptr<topology::node::Node>> &nodes)
{
    for (const auto &xml_node_pair : xml_nodes) {
        ...
        auto node = std::make_shared<topology::node::Node>(device_id, slotId, hostname, ip_infos, chip_num, die_num,
                                                           chip_type);
                                    ...
                                    std::vector<std::shared_ptr<topology::node::UbController>> &ub_controllers)
{
    for (const auto &[xml_iou_info_key, xml_iou_info] : xml_iou_infos) {
        ...
        uint32_t slotId = xml_iou_info.slotId;
        uint32_t chip_id = xml_iou_info.ubpuId;
        uint32_t iouId = xml_iou_info.iouId;
        std::string primaryCna = xml_iou_info.primaryCna;
        ...
        std::string ubc_state_str = lcne::common::StringToUpper(xmlLogicEntity->state);
        ...
        auto ub_controller = std::make_shared<topology::node::UbController>(
            die_guid, ubc_eid, device_id, slotId, chip_id, iouId, primaryCna, port_ids, die_state, ubc_state);
                            ...
                            std::vector<std::shared_ptr<topology::node::Port>> &ports)
{
    for (const auto &[xml_address_key, xml_address] : xml_addresses) {
        for (const auto &[port_id, physical_port] : xml_address.physicalPorts) {
            uint32_t physicalPortId = physical_port.physicalPortId;
            std::string port_cna = physical_port.portCna;
            std::string primaryCna = xml_address.primaryCna;
            ...
            auto node_port_it = node.physicalPorts.find(physicalPortId);
            ...
            auto port = std::make_shared<topology::node::Port>(physicalPortId, port_cna, primaryCna, device_id,
                                                               port_state, remote_physical_port_id, remote_slot,
                                                               remote_slot, remote_ubpu, remote_iou);
