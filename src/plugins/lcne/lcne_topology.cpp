...
namespace lcne::topo {
template <typename T>
std::vector<T> convertPtrVectorToValueVector(const std::vector<std::shared_ptr<T>> &ptrVec)
{
    std::vector<T> result;
...
LcneResult LcneTopology::CreateTopolgy()
{
    ...
    std::vector<topology::node::UbController> ubcs_vec = convertPtrVectorToValueVector(ubcs);
    std::vector<topology::node::Port> ports_vec = convertPtrVectorToValueVector(ports);
...
LcneResult LcneTopology::GetHisTopolgy(std::vector<std::shared_ptr<topology::node::Node>> &nodes,
                                       std::vector<std::shared_ptr<topology::node::UbController>> &ubcs,
                                       std::vector<std::shared_ptr<topology::node::Port>> &ports)
{
    LcneResult ret = nodeLocalCollectorModule->QueryHisDeviceData(nodes);
    if (ret != LCNE_SUCCESS) {
        LOG_ERROR << "LcneTopology::GetHisTopolgy-Error: failed to get his node topology";
        return ret;
    }
    ret = nodeLocalCollectorModule->QueryHisUbCData(ubcs);
    if (ret != LCNE_SUCCESS) {
        LOG_ERROR << "LcneTopology::GetHisTopolgy-Error: failed to get his ubc topology";
        return ret;
    }
    ret = nodeLocalCollectorModule->QueryHisPortData(ports);
    if (ret != LCNE_SUCCESS) {
        LOG_ERROR << "LcneTopology::GetHisTopolgy-Error: failed to get his port topology";
        return ret;
    }
    return LCNE_SUCCESS;
}
LcneResult LcneTopology::GetCurTopolgy(std::vector<std::shared_ptr<topology::node::Node>> &nodes,
                                       std::vector<std::shared_ptr<topology::node::UbController>> &ubcs,
                                       std::vector<std::shared_ptr<topology::node::Port>> &ports)
{
...
LcneResult LcneTopology::RegLinkNotifyHttpHandler()
{
    rack::com::RackHttpHandler linkNotifyHandlerFunc =
        std::bind(&LcneTopology::LinkNotifyHandlerFunc, this, std::placeholders::_1, std::placeholders::_2);
    rack::com::RackHttpServerHandler::GetInstance().Register(
        rack::com::RackHttpMethod::POST, lcne::common::LCNE_NOTIFY_TOPO_PATH, linkNotifyHandlerFunc);
    return LCNE_SUCCESS;
}
rack::com::RackComResult<rack::com::RackHttpResponse> LcneTopology::LinkNotifyHandlerFunc(
    ...
    const rack::com::RackComContext &ctx, const rack::com::RackHttpRequest &req)
{
    ...
    rack::com::RackHttpResponse resp;
    LcneResult ret = lcne::common::SaveIpToConfigFile(req.remote_addr + ":" + std::to_string(req.remote_port));
    if (ret != LCNE_SUCCESS) {
...
rack::com::RackComResult<rack::com::RackHttpResponse> LcneTopology::GetLcneTopologyHandlerFunc(
    const rack::com::RackComContext &ctx, const rack::com::RackHttpRequest &req)
{
    rack::com::RackHttpResponse resp;
    resp.status = 200;
