...
constexpr const char *LCNE_NODES_REQ_PATH = "/restconf/data/huawei-lingqu-topology:lingqu-topology/nodes";
constexpr const char *LCNE_ADDRESS_REQ_PATH = "/restconf/data/huawei-lingqu-topology:lingqu-topology/addresses";
constexpr const char *LCNE_IOU_INFOS_REQ_PATH = "/restconf/data/huawei-vbussw-service:vbussw-service/iou-infos";
constexpr const char *LCNE_LOGIC_ENTITIES_REQ_PATH =
    "/restconf/data/huawei-vbussw-inventory:vbussw-inventory/logic-entities";
constexpr const char *LCNE_NOTIFY_LINK_REQ_PATH = "/restconf/operations/notifications:create-subscription";
...
LcneResult convertTextToUint(tinyxml2::XMLElement *element, T &result)
{
    ...
    if (checkXML(element) == LCNE_FAIL) {
        LOG_ERROR << "convertTextToUint-Error: check xml failed, path: " << element->Name();
        return ret;
    }
    ...
    try {
        ...
        if (value > std::numeric_limits<T>::max()) {
            LOG_ERROR << "convertTextToUint-Error: value out of range, path: " << element->Name();
            return ret;
        }
        ret = LCNE_SUCCESS;
        ...
        LOG_ERROR << "convertTextToUint-Error: convert text to uint failed, path: " << element->Name()
                  << ", error: " << e.what();
        return ret;
...
inline LcneResult ConvertTextToOptionalUint(tinyxml2::XMLElement *element, std::optional<T> &result)
{
    ...
    LcneResult ret = LCNE_FAIL;
    if (checkXML(element) == LCNE_FAIL) {
        LOG_ERROR << "convertTextToOptionalUint-Error: check xml failed, path: " << element->Name();
        return ret;
    }
    const char *text = element->GetText();
    try {
        unsigned long value = std::stoul(std::string(text));
        if (value > std::numeric_limits<T>::max()) {
            LOG_ERROR << "convertTextToOptionalUint-Error: value out of range, path: " << element->Name();
            return ret;
        }
        ret = LCNE_SUCCESS;
        result = static_cast<T>(value);
        ...
        LOG_ERROR << "convertTextToOptionalUint-Error: convert text to uint failed, path: " << element->Name()
                  << ", error: " << e.what();
        return ret;
    }
}
