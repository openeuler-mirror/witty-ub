...
# ── 冻结契约：平铺 dict 的全部 31 键（与 _build_flat_trace_index 逐键对齐）──
...
LABEL_TO_COLUMNS: dict[str, tuple[str, ...]] = {
    SDK_LABEL: (
        ...
        "data_size", "inflight_count",
    ),
    WORKER_ACCESS_LABEL: ("worker_total_latency",),
    URMA_LABEL: ("urma_total_latency", "src", "dst"),
    REMOTE_PULL_LABEL: ("src", "dst"),
    LINK_LABEL: ("urma_link_latency",),
    QUERY_META_LABEL: ("worker_query_meta_latency",),
    SDK_PROCESS_LABEL: ("sdk_process",),
    SDK_RPC_LABEL: ("sdk_rpc",),
    LOCAL_WORKER_COST_LABEL: ("local_worker_cost",),
    LOCAL_WORKER_LOCK_LABEL: ("local_worker_lock",),
    REMOTE_WORKER_COST_LABEL: ("remote_worker_cost",),
    REMOTE_WORKER_RPC_LABEL: ("remote_worker_rpc",),
    MASTER_PROCESS_LABEL: ("master_process",),
    MASTER_RPC_LABEL: ("master_rpc_total",),
}
...
# src/dst 取源优先级：URMA=2 > RemotePull=1 > 其他 0
...
def _project(label: str, entry, row: dict[str, object]) -> None:
    ...
    if label == SDK_LABEL:
        ...
        row["op_key"] = "SET" if "SET" in op else "GET"
