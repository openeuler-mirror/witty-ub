...
async def parse_target(target: str):
    ...
    if target_path.exists():
        return await KVCacheLogParseWorker.parse_log(
            log_dir=str(target_path.resolve())
        )
...
async def prepare_run_target(target: str) -> tuple[str, bool]:
    ...
    if not await LogFilePGManager.add_log_file(log_file):
...
async def _run_profile(
    ...
    if command == "parse":
        async def operation() -> dict[str, object]:
            trace_index = await parse_target(args.target)
            return {"parse_results": len(trace_index.get("SDK access parse", []))}
        ...
        async def operation() -> dict[str, object]:
            ...
            return {
                "parse_results": len(trace_index.get("SDK access parse", [])),
                "anomalous_detail_rows": len(detail_rows),
            }
        ...
        async def operation() -> dict[str, object]:
            aggregates, _, time_window_aggregates, anom_tids = (
        ...
        async def operation() -> dict[str, object]:
            ...
            aggregate_started = time.perf_counter()
            aggregates, src_dst_map, time_window_aggregates, anom_tids = (
                await KVCacheLogParseWorker.generate_aggregate_result(trace_index)
            )
            aggregate_elapsed = time.perf_counter() - aggregate_started
            ...
            LogParseResultPGManager.profile_explicit_wal_checkpoint = True
            try:
                ...
                LogParseResultPGManager.profile_explicit_wal_checkpoint = False
            store_elapsed = time.perf_counter() - store_started
            store_timings = dict(LogParseResultPGManager.last_store_metrics)
