...
class TaskManager:
    ...
    @staticmethod
    async def get_tasks_by_ids(task_ids: list[str]) -> list[TaskModel]:
        """根据任务ID列表获取多个任务信息"""
        if not task_ids:
            return []
    ...
    async def create_task(task: TaskModel) -> bool:
        """创建新任务"""
        sql_str = """
            INSERT INTO task_table (id, pid, task_name, task_type, status, task_related_params, created_at)
            VALUES (:id, :pid, :task_name, :task_type, :status, :task_related_params, :created_at)
        """
    ...
    @staticmethod
    async def delete_tasks_by_ids(task_ids: list[str]) -> bool:
        """根据任务ID列表删除多个任务"""
        if not task_ids:
            return False
        placeholders = ", ".join(["?"] * len(task_ids))
