...
@router.post(
    ...
    description=(
        ...
        "（winner-take-all），固定顺序返回含零计数桶，桶间计数可直接比较、"
        "合计 = 采样数。GET 与 SET 为两套独立视角（读路径 8 桶 / 写路径 5 桶，"
        "key 不重叠），需分别调用、禁止混合归桶；其他 operation 返回空 items + note。"
        "样本集 = 异常 trace 全量 + top 慢正常（合并去重，"
        "sample_cap 截断）。"
