...
{
    OP_RET ret = db->CreateTable(
        "Device", vector<tuple<string, string, bool, bool>>(
                      {make_tuple("deviceId", "TEXT", true, true), make_tuple("slotId", "TEXT", true, false),
    ...
    ret = db->CreateTable(
        "Port", vector<tuple<string, string, bool, bool>>(
...
OP_RET NodeCollector::QueryHisNodeData(vector<std::shared_ptr<topology::node::Node>> &nodes)
{
...
OP_RET NodeCollector::QueryHisUbCData(vector<std::shared_ptr<topology::node::UbController>> &ubcs)
{
    std::unordered_map<std::string, std::pair<database::COMP_SYMB, std::string>> data;
    std::vector<std::unordered_map<std::string, std::string>> res;
    ...
    for (auto data_map : res) {
        ...
        ubcs.push_back(std::make_shared<UbController>(ubc.dieGuid, ubc.ubcEid, ubc.deviceId, ubc.slotId, ubc.chipId,
                                                      ubc.dieId, ubc.primaryCna, ubc.portIds, ubc.dieState,
                                                      ubc.ubcState));
    }
    return ret;
}
OP_RET NodeCollector::QueryHisPortData(vector<std::shared_ptr<topology::node::Port>> &ports)
{
    std::unordered_map<std::string, std::pair<database::COMP_SYMB, std::string>> data;
    ...
    for (auto data_map : res) {
        ...
        ports.push_back(std::make_shared<Port>(port.portId, port.portCna, port.primaryCna, port.deviceId,
                                               port.portState, port.remotePortId, port.remoteDeviceId,
                                               port.remoteSlotId, port.remoteUbpuId, port.remoteIouId));
    }
    return ret;
}
...
OP_RET NodeCollector::QueryCurUbCData(vector<std::shared_ptr<topology::node::UbController>> &ubcs)
{
    std::unordered_map<std::string, std::pair<database::COMP_SYMB, std::string>> data;
    std::vector<std::unordered_map<std::string, std::string>> res;
