...
public:
    ...
    void AddModuleDependencies()
    {
        ...
        for (std::type_index dep : module.GetDependencies()) {
            LOG_INFO << "UbseContext::AddModuleDependencies-Info: module " << typeid(T).name() << " depends on "
                     << dep.name();
        }
        return;
    }
    ...
    RackResult RegisterModule()
    {
        ...
        if (moduleDeps.find(typeid(T)) == moduleDeps.end()) {
        ...
        while (!moduleQueue.empty()) {
            ...
            if (initModules.find(currentModule) == initModules.end()) {
                ...
                for (auto dep : moduleDeps[currentModule]) {
                    ...
                    LOG_INFO << "UbseContext::RegisterModule-Info: module " << currentModule.name() << " depends on "
                             << dep.name();
                }
            }
        }
    ...
    TopoToolsArgs GetTopoToolsArgs()
    {
        return topoArgs;
    }
