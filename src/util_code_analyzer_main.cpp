...
#define MODULE_NAME "UTIL_CODE_ANALYZER"
...
using namespace code_analyzer;

UbseContext &g_rackContext = UbseContext::GetInstance();

void RegisterModules()
{
    g_rackContext.RegisterModule<CodeAnalyzerModule>();
}

void InitDependencies()
{
    g_rackContext.AddModuleDependencies<CodeAnalyzerModule>();
}
...
void CreateModules()
{
    ...
    for (auto type : sortedModules) {
        ...
        if (type == typeid(CodeAnalyzerModule)) {
            g_rackContext.InitModule<CodeAnalyzerModule>(RackModule::CreateModule<CodeAnalyzerModule>());
        } else {
