...
template <typename T>
string Vector2Str(vector<T> const &values)
{
...
struct Node : public ObjFormatter {
    ...
    Node(uint32_t deviceId, uint32_t slotId, string hostname, vector<string> ipAddrs, uint32_t chipNum, uint32_t dieNum,
         ChipType chipType)
        : deviceId(deviceId),
          slotId(slotId),
          hostname(hostname),
...
struct UbController : public ObjFormatter {
    ...
    UbController(string dieGuid, string ubcEid, uint32_t deviceId, uint32_t slotId, uint32_t chipId, uint32_t dieId,
                 string primaryCna, vector<uint32_t> portIds, DieState dieState, UbCState ubcState)
        : dieGuid(dieGuid),
          ubcEid(ubcEid),
          deviceId(deviceId),
          slotId(slotId),
...
struct Port : public ObjFormatter {
    ...
    Port() = default;
    Port(uint32_t portId, string portCna, string primaryCna, uint32_t deviceId, PortState portState,
         std::optional<uint32_t> remotePortId, std::optional<uint32_t> remoteDeviceId,
         std::optional<uint32_t> remoteSlotId, std::optional<uint32_t> remoteUbpuId,
         std::optional<uint32_t> remoteIouId)
        : portId(portId),
          portCna(portCna),
          primaryCna(primaryCna),
          ...
          remoteUbpuId(remoteUbpuId),
          remoteIouId(remoteIouId)
    {
    }
    unordered_map<string, string> ObjToDataMap() override
    {
        ...
        data_map["remotePortId"] = remotePortId ? to_string(remotePortId.value()) : "-";
        data_map["remoteDeviceId"] = remoteDeviceId ? to_string(remoteDeviceId.value()) : "-";
        data_map["remoteSlotId"] = remoteSlotId ? to_string(remoteSlotId.value()) : "-";
        data_map["remoteUbpuId"] = remoteUbpuId ? to_string(remoteUbpuId.value()) : "-";
        data_map["remoteIiId"] = remoteIouId ? to_string(remoteIouId.value()) : "-";
        return data_map;
    }
};
