...
{
    switch (opt) {
        case EventTypeOption::BIND:
            return "bind";
        case EventTypeOption::POST:
            return "post";
        case EventTypeOption::UNBIND:
            return "unbind";
        default:
            return "unknown";
    }
...
FailureMode FailureMode::FromJson(const Json::Value &j)
{
    ...
    std::vector<PathCell> pathCells = {{std::nullopt, j["log_path"].asString()}};
...
PathCell::PathCell(const std::optional<std::string> &podId, const std::string &path) : podId(podId), path(path) {}
...
std::optional<int64_t> DatetimeStrToTimestamp(const std::string &datetimeStr, bool allowFuture)
{
    auto validate_and_convert = [&](std::tm &t, int64_t us) -> std::optional<int64_t> {
        ...
        if (normalized.tm_year != orginal.tm_year || normalized.tm_mon != orginal.tm_mon ||
            normalized.tm_mday != orginal.tm_mday || normalized.tm_hour != orginal.tm_hour ||
            normalized.tm_min != orginal.tm_min || normalized.tm_sec != orginal.tm_sec) {
            return std::nullopt;
        }
