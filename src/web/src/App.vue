...
type FaultTraceTableRow = TraceDetailRow & {
  ...
  failureModes: FaultTraceFailureMode[]
  operation?: string
}

type FaultTraceFailureMode = {
  id: string
  name: string
  errorCode: string
  failureDomain: string
  detail: FailureModeKnowledgeModel | null
}
...
const activeAgentSessionTitle = computed(() =>
  agentSessions.value.find((session) => session.id === agentSessionId.value)?.title || '新会话',
)
...
const loadAgentSessionAssetIndex = () => {
  try {
    const stored = JSON.parse(window.localStorage.getItem(agentStorageKey(agentSessionIndexStorageKey)) || '{}')
...
const createAgentSession = async () => {
  ...
  try {
    ...
    agentSessions.value = [created, ...agentSessions.value.filter((item) => item.id !== created.id)]
    return created
  } finally {
...
const newAgentConversation = async () => {
  ...
  try {
    ...
    agentConnectionError.value = error instanceof Error ? error.message : '新建会话失败。'
  }
}
...
const openAgentSession = async (session: OpenCodeSession) => {
  ...
  try {
    ...
    const context = messages.find((message) => message.info.role === 'user')?.parts
    ...
    if (isAgentSending.value) {
      ...
      else agentChatMessages.value.push({
        id: nextAgentLocalMessageId(), role: 'assistant', reasoning: '', content: '',
        reasoningCollapsed: false, status: 'thinking',
      })
    }
    isAgentHistoryLoading.value = false
...
const submitAgentSessionDialog = async () => {
  ...
  try {
    if (kind === 'rename') {
      ...
      agentSessions.value = agentSessions.value.map((item) => item.id === session.id ? updated : item)
    ...
    if (statuses[session.id] && statuses[session.id]?.type !== 'idle') {
      await requestAgentApi<boolean>(`/session/${encodeURIComponent(session.id)}/abort`, { method: 'POST' })
    }
...
const selectDefaultAgentModel = () => {
  ...
  const orderedProviders = [...providers.all].sort((left, right) =>
...
const connectAgent = async (serverAddress: string, authHeader = '') => {
  ...
  try {
    ...
    let previousSession = ''
    try {
      previousSession = window.localStorage.getItem(agentStorageKey('witty-ub.active-session')) || ''
    } catch {
      // Browser storage is optional.
    }
    const restored = agentSessions.value.find((session) => session.id === previousSession)
    if (restored) await openAgentSession(restored)
    saveAgentConnection()
  } catch (error) {
...
const handleOpenCodeEvent = (event: MessageEvent<string>) => {
  ...
  if (payload.type === 'session.status' && ['busy', 'retry'].includes(properties?.status?.type || '')) {
  ...
  if (payload.type === 'message.updated' && properties?.info?.role === 'assistant') {
    ...
    if (messageId) {
      ...
      if (!pending && !agentChatMessages.value.some((message) => message.messageId === messageId)) {
        pending = {
          id: messageId, messageId, role: 'assistant', content: '', reasoning: '',
          reasoningCollapsed: false, status: 'thinking',
        }
    ...
    if (properties.info.error) markAgentResponseFailed(extractOpenCodeError(properties.info.error, '模型响应失败。'))
    return
  }

  if (payload.type === 'message.part.delta' && properties?.messageID && properties.partID) {
    const target = agentChatMessages.value.find((message) => message.messageId === properties.messageID)
  ...
  if (
    ...
    if (agentSessionId.value) {
      ...
      const sequence = agentRequestSequence
      void requestAgentApi<OpenCodeMessage[]>(`/session/${encodeURIComponent(agentSessionId.value)}/message`)
        .then((messages) => {
...
const replayAgentHistoryEvents = () => {
  ...
  for (const part of parts.values()) {
    ...
    if (!existing || (part.text || '').startsWith(existing.text)) {
      handleOpenCodeEvent(new MessageEvent('message', {
...
const sendAgentMessage = async () => {
  if (isAgentHistoryLoading.value || isAgentHistoryFailed.value || isAgentSessionCreating.value || isAgentSessionSaving.value || isAgentSubmitting.value) return
  if (isAgentSending.value) {
  ...
  try {
    ...
    const contextPrefix = conversationAssetId
      ...
      : ''
    const session = agentSessions.value.find((item) => item.id === sendingSessionId)
    if (session?.title === '新会话') {
      const updated = await requestAgentApi<OpenCodeSession>(`/session/${encodeURIComponent(sendingSessionId)}`, {
        method: 'PATCH', body: JSON.stringify({ title: question.slice(0, 60) }),
      })
      if (requestSequence !== agentRequestSequence) return
...
const syncFaultDetailTraceRowHeights = () => {
  nextTick(() => {
    syncSplitTableRowHeights(
      faultDetailTraceTableRef.value,
      '.fault-trace-scroll-grid.aggregate-body-row',
    )
  })
}
...
watch(
  [faultDetailTraceRows, isFaultDetailTraceEventsLoading],
  () => {
    // Rows are hidden while the request is loading, so measuring when the data is assigned
    // returns no DOM nodes. Re-sync after loading finishes and the three table sections render.
    if (!isFaultDetailTraceEventsLoading.value) syncFaultDetailTraceRowHeights()
  },
  { immediate: true },
)
...
const resizeLatencyCharts = () => {
  ...
  syncFaultTraceRowHeights()
  syncFaultDetailTraceRowHeights()
}
...
const isAccessTraceLog = (log: TraceLogRow) =>
  log.formatName === '接口日志' || Boolean(log.faultDomain?.trim())
...
const getTraceLogFailureModeLabels = (log: TraceLogRow) =>
  log.failureModeIds
    .map((id) => {
      return {
        id,
        label: getFailureModeLabel(id, isAccessTraceLog(log)),
      }
    })
...
const getAccessFailureModeIds = (trace?: TraceDetailRow | null) => {
  const ids = new Set<string>()
  getTraceLogs(trace).forEach((log) => {
    if (!isAccessTraceLog(log)) return
    log.failureModeIds.forEach((id) => ids.add(id))
  })
  return ids
}

const selectedTraceAccessFailureModeIds = computed(() =>
  getAccessFailureModeIds(selectedTrace.value),
)
...
const selectedFaultTraceAccessFailureModeIds = computed(() =>
  getAccessFailureModeIds(selectedFaultTrace.value),
)
...
const getRelatedFailureModeLabel = (failureModeId: string, isAccessFailure: boolean) => {
  if (!hasFailureModeDetailResult(failureModeId)) {
...
const fetchAccessFailureModeIdsByTrace = async (traceIds: string[], assetId: string) => {
  ...
  const normalizedTraceIds = [...new Set(traceIds.map((traceId) => traceId.trim()).filter(Boolean))]
  normalizedTraceIds.forEach((traceId) => accessFailureModeIdsByTrace.set(traceId, new Set()))
  if (normalizedTraceIds.length === 0) return accessFailureModeIdsByTrace
  ...
  const result = await request<{
    ...
    body: JSON.stringify({
      kb_id: assetId,
      trace_ids: normalizedTraceIds,
    }),
  })

  ;(result.log_failure_event_results ?? []).forEach((event) => {
    const traceId = event.trace_id?.trim()
    if (!traceId) return
    const log = toTraceLogRow(event)
    if (!isAccessTraceLog(log)) return
    const accessFailureModeIds = accessFailureModeIdsByTrace.get(traceId) ?? new Set<string>()
    log.failureModeIds.forEach((failureModeId) => accessFailureModeIds.add(failureModeId))
    accessFailureModeIdsByTrace.set(traceId, accessFailureModeIds)
  })

  return accessFailureModeIdsByTrace
}

const toFaultTraceTableRow = (
  result: TraceFailureEventResultModel,
  accessFailureModeIds: Set<string>,
): FaultTraceTableRow => {
  const record = result as Record<string, unknown>
  ...
  const failureModeIds = getFailureModeIds(record)
  const failureModes = failureModeIds.map((id) => {
    const detail = failureModeDetailsById.value[id] ?? null
    const errorCode = normalizeFailureModeErrorCode(detail?.error_code)
    return {
      id,
      name: detail?.name || id,
      errorCode,
      failureDomain: detail?.failure_domain || '-',
      detail,
    }
  })
  // Trace 汇总事件会同时包含 access 与 runtime 故障。外层列表的故障码只展示
  // 原始 access 日志实际命中的故障模式对应的 error_code。
  const faultCodes = [
    ...new Set(
      [...accessFailureModeIds]
        .map((failureModeId) =>
          normalizeFailureModeErrorCode(failureModeDetailsById.value[failureModeId]?.error_code),
        )
        .filter(Boolean),
    ),
  ]
  ...
  return {
    ...
    masterRpcTotal: null,
    faultCodes,
    faultType: failureModes.length > 0 ? failureModes.map((mode) => mode.name).join('、') : '-',
    faultDomain: failureDomains.length > 0 ? failureDomains.join('、') : '-',
    failureDomains,
    failureModes,
...
const loadFaultAggregatedEventDetailTraceEvents = async (
  ...
  try {
    ...
    const traceIds = events.map((event) => event.trace_id).filter((id): id is string => !!id)
    const accessFailureModeIdsByTrace = await fetchAccessFailureModeIdsByTrace(traceIds, assetId)
    const accessFailureModeIds = [...accessFailureModeIdsByTrace.values()].flatMap((ids) => [
      ...ids,
    ])
    await Promise.all(
      [...new Set([...failureModeIds, ...accessFailureModeIds])].map((failureModeId) =>
        loadFailureModeDetail(failureModeId),
      ),
    )
    ...
    if (
      ...
      faultDetailTraceRows.value = events.map((event) =>
        toFaultTraceTableRow(
          event,
          accessFailureModeIdsByTrace.get(event.trace_id ?? '') ?? new Set<string>(),
        ),
      )
...
const loadFaultTraceEvents = async (pageNum = faultTraceEventsPage.value) => {
  ...
  try {
    ...
    faultTraceRowsMap[scale] = events.map((event) =>
      toFaultTraceTableRow(
        event,
        accessFailureModeIdsByTrace.get(event.trace_id ?? '') ?? new Set<string>(),
      ),
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          ...
                          <div class="aggregate-cell fault-trace-multi-value-cell">
                            <span
                              v-for="code in trace.faultCodes"
                              :key="`${trace.id}-fault-${code}`"
                              class="fault-code-pill fault-code-clickable"
                              @click="openStatusCodePopover(code, $event)"
                            >
                              {{ code }}
                            </span>
                            <span v-if="trace.faultCodes.length === 0" class="fault-code-pill"
                              >-</span
                            >
                          </div>
                          <div class="aggregate-cell fault-trace-multi-value-cell">
                            <template
                              v-for="failureMode in trace.failureModes"
                              :key="`${trace.id}-failure-mode-${failureMode.id}`"
                            >
                              <button
                                v-if="failureMode.detail"
                                class="failure-mode-link-btn"
                                type="button"
                                @click="openFailureModeDetailPopover(failureMode.detail, $event)"
                              >
                                {{ failureMode.name }}
                              </button>
                              <span v-else>{{ failureMode.name }}</span>
                            </template>
                            <span v-if="trace.failureModes.length === 0">-</span>
                          </div>
                          <div class="aggregate-cell fault-trace-multi-value-cell">
                            <span
                              v-for="failureDomain in trace.failureDomains"
                              :key="`${trace.id}-failure-domain-${failureDomain}`"
                            >
                              {{ failureDomain }}
                            </span>
                            <span v-if="trace.failureDomains.length === 0">-</span>
                          </div>
                        </div>
                      </template>
                    </div>
    ...
    <div v-if="selectedTrace" class="modal" role="dialog" aria-modal="true">
      <section class="modal-content trace-modal-content">
        ...
        <div class="modal-body trace-modal-body">
          ...
          <section>
            <h3 class="trace-section-title">🗃️ 故障模式详情</h3>
            <div v-if="selectedTraceFailureModeIds.length === 0" class="trace-fault-detail-list">
              <div class="trace-fault-detail-item trace-fault-detail-wide">
            ...
            <div v-else-if="!selectedTraceFailureMode" class="failure-mode-chain-list">
              <div class="failure-mode-chain-item">
                <div class="failure-mode-chain-detail">
                  <span class="trace-fault-detail-value">
                    {{ getFailureModeDetailStateLabel(selectedTraceFailureModeId) }}
                  </span>
                </div>
              </div>
            </div>
                    ...
                    <div class="trace-fault-detail-item trace-fault-detail-wide">
                      ...
                      <div v-else class="trace-sub-fault-block">
                        ...
                        <div v-if="selectedChildFailureModeId" class="trace-sub-fault-detail">
                          ...
                          <span v-else class="trace-fault-detail-value">暂无相关故障详情</span>
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          <section>
            ...
            <div v-else-if="!selectedFaultTraceFailureMode" class="failure-mode-chain-list">
              <div class="failure-mode-chain-item">
                <div class="failure-mode-chain-detail">
                  <span class="trace-fault-detail-value">
                    {{ getFailureModeDetailStateLabel(selectedFaultTraceFailureModeId) }}
                  </span>
                </div>
              </div>
            </div>
      ...
      <div v-if="agentView === 'chat'" class="agent-conversation-layout">
        <aside class="agent-session-manager" aria-label="会话列表">
          <div class="agent-session-manager-title">
            ...
            <button type="button" :disabled="isAgentSessionCreating || isAgentSessionSaving || isAgentSubmitting" @click="newAgentConversation">
              {{ isAgentSessionCreating ? '创建中…' : '＋ 新建' }}
            </button>
          </div>
          <button class="agent-session-refresh" type="button" :disabled="isAgentSessionsLoading || isAgentSessionCreating || isAgentSessionSaving" @click="loadAgentSessions">刷新会话列表</button>
          <input
          ...
          <div class="agent-session-list">
            ...
            >
              <button class="agent-session-open" type="button" :disabled="isAgentSessionCreating || isAgentSessionSaving || isAgentSubmitting" :aria-current="session.id === agentSessionId ? 'true' : undefined" @click="openAgentSession(session)">
                <strong>{{ session.title || '无标题会话' }}</strong>
                <span>{{ getAgentSessionAssetName(session.id) }}</span>
                <span>{{ agentSessionStatuses[session.id]?.type === 'busy' ? '正在回答' : agentSessionStatuses[session.id]?.type === 'retry' ? '正在重试连接' : session.time?.updated ? new Date(session.time.updated).toLocaleString() : '刚刚创建' }}</span>
              </button>
              <div class="agent-session-item-actions">
                <button type="button" title="修改标题" :aria-label="`重命名会话 ${session.title || '无标题会话'}`" :disabled="isAgentSessionSaving || isAgentSubmitting" @click="showAgentSessionDialog('rename', session)">
                  ✎
                </button>
                <button type="button" title="删除会话" :aria-label="`删除会话 ${session.title || '无标题会话'}`" :disabled="isAgentSessionSaving || isAgentSubmitting" @click="showAgentSessionDialog('delete', session)">
                  ×
                </button>
              </div>
            </article>
        ...
        <section class="agent-conversation-main">
          <div class="agent-conversation-context">
            ...
            <span>{{ agentSessionId ? getAgentSessionAssetName(agentSessionId) : selectedAsset?.name || '请先选择资产库' }}</span>
          </div>
          <form v-if="agentSessionDialog" class="agent-session-dialog" role="dialog" aria-modal="false" :aria-label="agentSessionDialog.kind === 'rename' ? '重命名会话' : '删除会话'" @submit.prevent="submitAgentSessionDialog" @keydown.esc="!isAgentSessionSaving && (agentSessionDialog = null)">
            <strong>{{ agentSessionDialog.kind === 'rename' ? '重命名会话' : '删除会话' }}</strong>
            <label v-if="agentSessionDialog.kind === 'rename'">会话标题
              <input v-model="agentSessionTitleInput" aria-label="会话标题" maxlength="200" :disabled="isAgentSessionSaving" />
            </label>
            <p v-else>确认删除「{{ agentSessionDialog.session.title || '无标题会话' }}」及其历史消息？正在运行的回答会停止，此操作无法撤销。</p>
            <p v-if="agentSessionDialogError" role="alert">{{ agentSessionDialogError }}</p>
            <div>
              <button type="button" :disabled="isAgentSessionSaving" @click="agentSessionDialog = null">取消</button>
              <button type="submit" :disabled="isAgentSessionSaving">{{ isAgentSessionSaving ? '保存中…' : agentSessionDialog.kind === 'rename' ? '保存标题' : '确认删除' }}</button>
            </div>
          </form>
          ...
          <div v-if="agentConnectionError" class="agent-chat-error" role="alert">
            {{ agentConnectionError }}
            <button v-if="agentSessionId" type="button" @click="openAgentSession({ id: agentSessionId })">重新加载会话</button>
          </div>
          ...
          <form class="agent-chat-composer" @submit.prevent="sendAgentMessage">
            <textarea
              ...
              :disabled="isAgentSending || isAgentAborting || isAgentHistoryLoading || isAgentHistoryFailed || isAgentSessionCreating || isAgentSessionSaving"
            ...
            <button
              ...
              :disabled="
                isAgentHistoryLoading || isAgentHistoryFailed || isAgentSessionCreating || isAgentSessionSaving || isAgentSubmitting || (isAgentSending ? isAgentAborting : !agentChatInput.trim() || isAgentAborting)
              "
            >
