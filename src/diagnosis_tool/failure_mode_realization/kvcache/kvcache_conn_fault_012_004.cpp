...
bool KvcacheConnFault012_004::IsValid()
{
    ...
    std::string grepOutput = urma_log_helper::RunCommand(
        "test -n \"$WITTY_UB_FAULT_LOG\" && grep -E 'Cannot receive heartbeat from worker|\\[HealthCheck\\] Worker is exiting now|meta_is_moving' \"$WITTY_UB_FAULT_LOG\"/datasystem_worker.INFO.log \"$WITTY_UB_FAULT_LOG\"/ds_client_*.INFO.log 2>/dev/null | tail -20");
