...
{
    return "业务参数非法，属于用户侧问题（08手册:L247）。";
}
...
{
    return "业务方校验调用参数（08手册:L247）。";
}

std::string KvcacheConnFault002_001::GetValidationMethodDesc() const
{
    return "通过日志关键字识别（08手册:L246-249）：匹配The objectKey "
           "is empty / dataSize should be bigger than zero / length not match。";
}
