...
bool KvcacheConnFault006::IsValid()
{
    ...
    std::string grepOutput =
        kvcache_log_helper::RunCommand("test -n \"$WITTY_UB_CLIENT_ACCESS_LOG$WITTY_UB_WORKER_ACCESS_LOG\" && awk "
                                       ...
                                       "$WITTY_UB_WORKER_ACCESS_LOG | sort | uniq -c | grep -E '[[:space:]]5$'");
    // 来源: rule f - 获取原始日志行用于trace解析，提取status_code=5的access log行
    std::string rawOutput =
        kvcache_log_helper::RunCommand("test -n \"$WITTY_UB_CLIENT_ACCESS_LOG$WITTY_UB_WORKER_ACCESS_LOG\" && awk "
                                       "-F'|' 'NR>0 {code=$8; gsub(/^ +| +$/,\"\",code)} code == \"5\" {print $0}' "
...
{
    // 来源: .opencode/skills/kvcache-diagnosis-conn-fault-code-generalizer/references/kvcache_conn_fault_mode.md:L284, L286, L131
    return "通过access log识别（来源：08手册:L131, L191-195）：access log中status_code（第8列）为5；并通过Worker INFO "
           "log按日志串细分。（来源：08手册:L131, L191-195）";
}
