...
bool KvcacheConnFault009::IsValid()
{
    ...
    std::string grepOutput =
        kvcache_log_helper::RunCommand("test -n \"$WITTY_UB_CLIENT_ACCESS_LOG$WITTY_UB_WORKER_ACCESS_LOG\" && awk "
                                       ...
                                       "$WITTY_UB_WORKER_ACCESS_LOG | sort | uniq -c | grep -E '[[:space:]]13$'");
    // 来源: rule f - 获取原始日志行用于trace解析，提取status_code=13的access log行
    std::string rawOutput =
        kvcache_log_helper::RunCommand("test -n \"$WITTY_UB_CLIENT_ACCESS_LOG$WITTY_UB_WORKER_ACCESS_LOG\" && awk "
                                       "-F'|' 'NR>0 {code=$8; gsub(/^ +| +$/,\"\",code)} code == \"13\" {print $0}' "
...
std::string KvcacheConnFault009::GetFixSuggDesc() const
{
    // 来源: .opencode/skills/kvcache-diagnosis-conn-fault-code-generalizer/references/kvcache_conn_fault_mode.md:L405, L407, L132
    return "df "
           "-h确认磁盘使用情况；检查resource.log中SPILL_HARD_DISK/"
           "SHARED_DISK字段；清理磁盘或扩容。（来源：08手册:L332）";
}

std::string KvcacheConnFault009::GetValidationMethodDesc() const
{
    // 来源: .opencode/skills/kvcache-diagnosis-conn-fault-code-generalizer/references/kvcache_conn_fault_mode.md:L405, L407, L132
    return "通过access log识别（来源：08手册:L132, L196）：access log中status_code（第8列）为13；或df "
           "-h检查磁盘满。（来源：08手册:L132, L332）";
}
