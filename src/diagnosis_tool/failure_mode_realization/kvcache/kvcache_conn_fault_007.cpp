...
bool KvcacheConnFault007::IsValid()
{
    ...
    std::string grepOutput =
        kvcache_log_helper::RunCommand("test -n \"$WITTY_UB_CLIENT_ACCESS_LOG$WITTY_UB_WORKER_ACCESS_LOG\" && awk "
                                       ...
                                       "$WITTY_UB_WORKER_ACCESS_LOG | sort | uniq -c | grep -E '[[:space:]]6$'");
    // 来源: rule f - 获取原始日志行用于trace解析，提取status_code=6的access log行
    std::string rawOutput =
        kvcache_log_helper::RunCommand("test -n \"$WITTY_UB_CLIENT_ACCESS_LOG$WITTY_UB_WORKER_ACCESS_LOG\" && awk "
                                       "-F'|' 'NR>0 {code=$8; gsub(/^ +| +$/,\"\",code)} code == \"6\" {print $0}' "
...
{
    // 来源: .opencode/skills/kvcache-diagnosis-conn-fault-code-generalizer/references/kvcache_conn_fault_mode.md:L362, L364, L132
    return "通过access log识别（来源：08手册:L132, L196）：access log中status_code（第8列）为6；或dmesg中Out of "
           "memory。（来源：08手册:L132, L330）";
}
