...
bool UrmaFailure660::IsValid()
{
    std::string grepOutput = urma_log_helper::RunCommand(
        R"(test -n "$URMA_LOG_PATH" && grep -F 'urma_cmd_wait_jfc' "$URMA_LOG_PATH" 2>/dev/null | grep -F 'Invalid parameter')");
...
{
    return "urma_cmd_wait_jfc 校验 JFCE 无效导致等待流程拒绝继续执行";
}

std::string UrmaFailure660::GetRootCauseDesc() const
{
    return "urma_cmd_wait_jfc 在执行等待前发现调用方传入的 JFCE 不满足当前操作要求，通常是对象为空、状态不匹配或与 "
