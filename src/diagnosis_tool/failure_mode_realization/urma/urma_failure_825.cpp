...
bool UrmaFailure825::IsValid()
{
    std::string grepOutput = urma_log_helper::RunCommand(
        R"(test -n "$URMA_LOG_PATH" && grep -F 'urma_validate_driver' "$URMA_LOG_PATH" 2>/dev/null | grep -F 'Invalid driver name length')");
...
{
    return "urma_validate_driver 校验 URMA 对象 无效导致处理流程拒绝继续执行";
}

std::string UrmaFailure825::GetRootCauseDesc() const
{
    return "urma_validate_driver 在执行处理前发现调用方传入的 URMA 对象 "
...
{
    return "在 URMA_LOG_PATH 中匹配关键日志：Invalid driver name length";
}
