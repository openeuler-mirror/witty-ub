...
bool UrmaFailure165::IsValid()
{
    std::string grepOutput = urma_log_helper::RunCommand(
        R"(test -n "$URMA_LOG_PATH" && grep -F 'urma_modify_tp' "$URMA_LOG_PATH" 2>/dev/null | grep -F 'Invalid parameter')");
...
{
    return "urma_modify_tp 校验 context 无效导致修改流程拒绝继续执行";
}

std::string UrmaFailure165::GetRootCauseDesc() const
{
    return "urma_modify_tp 在执行修改前发现调用方传入的 context 不满足当前操作要求，通常是对象为空、状态不匹配或与 "
