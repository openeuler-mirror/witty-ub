...
bool UrmaFailure050::IsValid()
{
    std::string grepOutput = urma_log_helper::RunCommand(
        R"(test -n "$URMA_LOG_PATH" && grep -F 'urma_cmd_import_jfr_ex' "$URMA_LOG_PATH" 2>/dev/null | grep -F 'Invalid parameter')");
...
{
    return "urma_cmd_import_jfr_ex 校验 context 无效导致导入流程拒绝继续执行";
}

std::string UrmaFailure050::GetRootCauseDesc() const
{
    return "urma_cmd_import_jfr_ex 在执行导入前发现调用方传入的 context "
