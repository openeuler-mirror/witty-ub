...
bool UrmaFailure127::IsValid()
{
    std::string grepOutput = urma_log_helper::RunCommand(
        ...
        "grep -F 'urma_flush_jetty' \"$URMA_LOG_PATH\" 2>/dev/null | "
...
{
    return "urma_flush_jetty 校验 context 无效导致处理流程拒绝继续执行";
}

std::string UrmaFailure127::GetRootCauseDesc() const
{
    return "urma_flush_jetty 在执行处理前发现调用方传入的 context 不满足当前操作要求，通常是对象为空、状态不匹配或与 "
