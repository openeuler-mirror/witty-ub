...
bool UrmaFailure586::IsValid()
{
    std::string grepOutput = urma_log_helper::RunCommand(
        "test -n \"$URMA_LOG_PATH\" && grep -F 'urma_step_perf' \"$URMA_LOG_PATH\" 2>/dev/null | "
        "grep -F 'Urma perf type' | grep -F 'is invalid.'");
...
{
    return "通过 URMA 日志关键字校验：urma_step_perf，Urma perf type，is invalid.";
}
