#include "urma_failure_072.h"

#include "../../failure_mode_factory.h"

namespace diag {
static AutoRegister<UrmaFailure072> g_urma("urma_072");

bool UrmaFailure072::IsValid(const std::vector<std::string> &fields)
{
    const std::string &message = fields[7];
    return message.find("urma_cmd_import_jfr") != std::string::npos &&
           message.find("Invalid parameter") != std::string::npos;
}

std::string UrmaFailure072::GetName() const
{
    return "URMA context、dev_fd、tjfr、配置参数无效导致导入JFR失败";
}

std::string UrmaFailure072::GetRootCauseDesc() const
{
    return "urma_cmd_import_jfr用于导入JFR，调用方传入的URMA "
           "context、dev_fd、tjfr、配置参数不满足接口前置条件，函数无法继续执行。";
}

RootCause UrmaFailure072::AnalyzeRootCause()
{
    return RootCause(true, GetRootCauseDesc());
}

std::string UrmaFailure072::GetFixSuggDesc() const
{
    return "无";
}

std::string UrmaFailure072::GetValidationMethodDesc() const
{
    return "通过 URMA_LOG_PATH 日志匹配关键字：urma_cmd_import_jfr，Invalid parameter。";
}

std::string UrmaFailure072::GetId() const
{
    return "urma_072";
}
} // namespace diag
