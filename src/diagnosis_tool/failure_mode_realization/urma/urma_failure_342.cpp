...
bool UrmaFailure342::IsValid()
{
    std::string grepOutput = urma_log_helper::RunCommand(
        R"(test -n "$URMA_LOG_PATH" && grep -F 'urma_cmd_set_jfr_opt' "$URMA_LOG_PATH" 2>/dev/null | grep -F 'Invalid parameter')");
...
{
    return "urma_cmd_set_jfr_opt 校验 context 无效导致设置流程拒绝继续执行";
}

std::string UrmaFailure342::GetRootCauseDesc() const
{
    return "urma_cmd_set_jfr_opt 在执行设置前发现调用方传入的 context "
