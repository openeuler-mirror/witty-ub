...
bool UrmaFailure035::IsValid()
{
    std::string grepOutput = urma_log_helper::RunCommand(
        R"(test -n "$URMA_LOG_PATH" && grep -F 'urma_cmd_active_jfs' "$URMA_LOG_PATH" 2>/dev/null | grep -F 'Invalid parameter')");
...
{
    return "urma_cmd_active_jfs 校验 context 无效导致激活流程拒绝继续执行";
}

std::string UrmaFailure035::GetRootCauseDesc() const
{
    return "urma_cmd_active_jfs 在执行激活前发现调用方传入的 context "
