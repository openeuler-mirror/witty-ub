...
bool UrmaFailure121::IsValid()
{
    std::string grepOutput = urma_log_helper::RunCommand(
        R"(test -n "$URMA_LOG_PATH" && grep -F 'urma_active_jfr' "$URMA_LOG_PATH" 2>/dev/null | grep -F 'jfr state is wrong in active_jfr')");
...
{
    return "在 URMA_LOG_PATH 中匹配关键日志：jfr state is wrong in active_jfr";
}
