...
{
    return "URMA context、Jetty对象无效导致创建Jetty失败";
}

std::string UrmaFailure126::GetRootCauseDesc() const
{
    return "函数用于创建Jetty，调用方传入的URMA context、Jetty对象不满足接口前置条件，无法继续完成本次URMA操作。";
}
...
{
    return "通过 URMA 日志关键字校验：urma_cmd_create_jetty，Invalid parameter";
}
