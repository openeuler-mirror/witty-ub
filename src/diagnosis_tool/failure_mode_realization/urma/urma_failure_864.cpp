...
bool UrmaFailure864::IsValid()
{
    std::string grepOutput = urma_log_helper::RunCommand(
        R"(test -n "$URMA_LOG_PATH" && grep -F 'deepcopy_jfs_wr_node' "$URMA_LOG_PATH" 2>/dev/null | grep -F 'Deepcopy src sg failed')");
...
{
    return "在 URMA_LOG_PATH 中匹配关键日志：Deepcopy src sg failed";
}
