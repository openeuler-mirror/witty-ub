...
bool UrmaFailure242::IsValid()
{
    std::string grepOutput = urma_log_helper::RunCommand(
        R"(test -n "$URMA_LOG_PATH" && grep -F 'set_jfr_wr_ptjetty_ptseg_without_hdr' "$URMA_LOG_PATH" 2>/dev/null | grep -F 'Recv sge[' | grep -F '] has NULL tseg')");
...
{
    return "set_jfr_wr_ptjetty_ptseg_without_hdr 执行设置 目标 Jetty 失败导致当前资源状态无法推进";
}

std::string UrmaFailure242::GetRootCauseDesc() const
{
    return "set_jfr_wr_ptjetty_ptseg_without_hdr 调用下层 provider、bond 组件或系统接口处理 目标 Jetty "
...
{
    return "在 URMA_LOG_PATH 中匹配关键日志：Recv sge[] has NULL tseg";
}
