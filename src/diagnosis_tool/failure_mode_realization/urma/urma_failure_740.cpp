...
bool UrmaFailure740::IsValid()
{
    std::string grepOutput = urma_log_helper::RunCommand(
        R"(test -n "$URMA_LOG_PATH" && grep -F 'urma_cmd_delete_jfc' "$URMA_LOG_PATH" 2>/dev/null | grep -F 'There is jfc event and it must be acked, jfc_comp:, comp:, jfc_async:, async')");
