...
bool UrmaFailure516::IsValid()
{
    std::string grepOutput = urma_log_helper::RunCommand(
        ...
        "grep -F 'Failed to delete pseg for vseg, token_id:' | grep -F ', handle:' | grep -F 'u.'");
...
{
    return "通过 URMA 日志关键字校验：bondp_unregister_seg_inner，Failed to delete pseg for vseg, token_id:，, "
           "handle:，u.";
}
