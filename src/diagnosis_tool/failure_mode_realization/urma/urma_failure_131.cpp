...
bool UrmaFailure131::IsValid()
{
    std::string grepOutput = urma_log_helper::RunCommand(
        R"(test -n "$URMA_LOG_PATH" && grep -F 'urma_bind_jetty' "$URMA_LOG_PATH" 2>/dev/null | grep -F 'Invalid parameter')");
...
{
    return "urma_bind_jetty 校验 目标 Jetty 无效导致绑定流程拒绝继续执行";
}

std::string UrmaFailure131::GetRootCauseDesc() const
{
    return "urma_bind_jetty 在执行绑定前发现调用方传入的 目标 Jetty 不满足当前操作要求，通常是对象为空、状态不匹配或与 "
