...
bool UrmaFailure028::IsValid()
{
    std::string grepOutput = urma_log_helper::RunCommand(
        R"(test -n "$URMA_LOG_PATH" && grep -F 'urma_cmd_set_jfs_opt' "$URMA_LOG_PATH" 2>/dev/null | grep -F 'jfc not exist in jfs')");
...
{
    return "在 URMA_LOG_PATH 中匹配关键日志：jfc not exist in jfs";
}
