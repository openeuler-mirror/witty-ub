...
bool UrmaFailure379::IsValid()
{
    std::string grepOutput = urma_log_helper::RunCommand(
        R"(test -n "$URMA_LOG_PATH" && grep -F 'urma_create_jfs' "$URMA_LOG_PATH" 2>/dev/null | grep -F 'Invalid parameter, trans_mode')");
...
{
    return "urma_create_jfs 校验 JFS 无效导致创建流程拒绝继续执行";
}

std::string UrmaFailure379::GetRootCauseDesc() const
{
    return "urma_create_jfs 在执行创建前发现调用方传入的 JFS 不满足当前操作要求，通常是对象为空、状态不匹配或与 "
