...
bool UrmaFailure820::IsValid()
{
    std::string grepOutput = urma_log_helper::RunCommand(
        R"(test -n "$URMA_LOG_PATH" && grep -F 'urma_cmd_modify_jetty' "$URMA_LOG_PATH" 2>/dev/null | grep -F 'Invalid parameter')");
...
{
    return "urma_cmd_modify_jetty 校验 context 无效导致修改流程拒绝继续执行";
}

std::string UrmaFailure820::GetRootCauseDesc() const
{
    return "urma_cmd_modify_jetty 在执行修改前发现调用方传入的 context "
