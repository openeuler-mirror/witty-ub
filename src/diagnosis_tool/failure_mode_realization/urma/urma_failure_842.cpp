...
bool UrmaFailure842::IsValid()
{
    std::string grepOutput = urma_log_helper::RunCommand(
        R"(test -n "$URMA_LOG_PATH" && grep -F 'bondp_del_jetty_p_vjetty_info_without_lock' "$URMA_LOG_PATH" 2>/dev/null | grep -F 'Failed to delete p_vjetty_id node: ret:' | grep -F 'pjetty_id:')");
