...
bool UrmaFailure676::IsValid()
{
    std::string grepOutput = urma_log_helper::RunCommand(
        R"(test -n "$URMA_LOG_PATH" && grep -F 'urma_ack_async_event' "$URMA_LOG_PATH" 2>/dev/null | grep -F 'Invalid parameter')");
...
{
    return "urma_ack_async_event 校验 context 无效导致确认流程拒绝继续执行";
}

std::string UrmaFailure676::GetRootCauseDesc() const
{
    return "urma_ack_async_event 在执行确认前发现调用方传入的 context "
