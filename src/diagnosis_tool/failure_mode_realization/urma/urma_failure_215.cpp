...
bool UrmaFailure215::IsValid()
{
    std::string grepOutput = urma_log_helper::RunCommand(
        R"(test -n "$URMA_LOG_PATH" && grep -F 'bondp_segment_get_args_list' "$URMA_LOG_PATH" 2>/dev/null | grep -F 'Failed to alloc args')");
...
{
    return "bondp_segment_get_args_list 分配 segment 临时参数失败导致获取流程无法继续";
}

std::string UrmaFailure215::GetRootCauseDesc() const
{
    return "bondp_segment_get_args_list 需要为 segment 构造命令参数、资源描述或临时缓存，但内存分配返回失败，后续 "
