...
bool UrmaFailure015::IsValid()
{
    std::string grepOutput = urma_log_helper::RunCommand(
        R"(test -n "$URMA_LOG_PATH" && grep -F 'bondp_import_jetty' "$URMA_LOG_PATH" 2>/dev/null | grep -F 'Single-path jetty only support RC, trans_mode:')");
...
{
    return "在 URMA_LOG_PATH 中匹配关键日志：Single-path jetty only support RC, trans_mode";
}
