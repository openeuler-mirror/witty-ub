...
bool UrmaFailure381::IsValid()
{
    std::string grepOutput = urma_log_helper::RunCommand(
        R"(test -n "$URMA_LOG_PATH" && grep -F 'urma_create_jfs' "$URMA_LOG_PATH" 2>/dev/null | grep -F 'jfs cfg out of range, depth:' | grep -F ', max_depth:' | grep -F ', inline_data:' | grep -F ', max_inline_len:' | grep -F ', sge:' | grep -F ', max_sge:' | grep -F ', rsge:' | grep -F ', max_rsge:')");
