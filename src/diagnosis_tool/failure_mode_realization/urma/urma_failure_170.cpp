...
bool UrmaFailure170::IsValid()
{
    std::string grepOutput = urma_log_helper::RunCommand(
        "test -n \"$URMA_LOG_PATH\" && grep -F 'urma_cmd_get_tp_list' \"$URMA_LOG_PATH\" 2>/dev/null | "
...
{
    return "URMA context、Jetty对象无效导致获取TP失败";
}

std::string UrmaFailure170::GetRootCauseDesc() const
{
    return "函数用于获取TP，调用方传入的URMA context、Jetty对象不满足接口前置条件，无法继续完成本次URMA操作。";
}
...
{
    return "通过 URMA 日志关键字校验：urma_cmd_get_tp_list，Invalid parameter.";
}
