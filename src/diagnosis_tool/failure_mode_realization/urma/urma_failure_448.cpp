...
bool UrmaFailure448::IsValid()
{
    std::string grepOutput = urma_log_helper::RunCommand(
        R"(test -n "$URMA_LOG_PATH" && grep -F 'urma_set_jetty_opt' "$URMA_LOG_PATH" 2>/dev/null | grep -F 'UB dev should use share jfr!')");
...
{
    return "urma_set_jetty_opt 执行设置 context 失败导致当前资源状态无法推进";
}

std::string UrmaFailure448::GetRootCauseDesc() const
{
    return "urma_set_jetty_opt 调用下层 provider、bond 组件或系统接口处理 context 时返回失败，当前分支携带 ret/errno "
