...
{
    return "bondp_create_vjfs 执行创建 context 失败导致当前资源状态无法推进";
}

std::string UrmaFailure177::GetRootCauseDesc() const
{
    return "bondp_create_vjfs 调用下层 provider、bond 组件或系统接口处理 context 时返回失败，当前分支携带 ret/errno "
...
{
    return "在 URMA_LOG_PATH 中匹配关键日志：ubcore create jfs failed";
}
