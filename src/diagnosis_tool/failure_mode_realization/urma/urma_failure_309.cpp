...
bool UrmaFailure309::IsValid()
{
    std::string grepOutput = urma_log_helper::RunCommand(
        R"(test -n "$URMA_LOG_PATH" && grep -F 'urma_cmd_alloc_token_id_ex' "$URMA_LOG_PATH" 2>/dev/null | grep -F 'Invalid parameter')");
...
{
    return "urma_cmd_alloc_token_id_ex 校验 context 无效导致分配流程拒绝继续执行";
}

std::string UrmaFailure309::GetRootCauseDesc() const
{
    return "urma_cmd_alloc_token_id_ex 在执行分配前发现调用方传入的 context "
