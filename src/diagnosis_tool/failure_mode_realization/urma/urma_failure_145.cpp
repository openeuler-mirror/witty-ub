...
bool UrmaFailure145::IsValid()
{
    std::string grepOutput = urma_log_helper::RunCommand(
        ...
        "grep -F 'urma_bind_jetty_async' \"$URMA_LOG_PATH\" 2>/dev/null | "
...
{
    return "urma_bind_jetty_async 校验 context 无效导致绑定流程拒绝继续执行";
}

std::string UrmaFailure145::GetRootCauseDesc() const
{
    return "urma_bind_jetty_async 在执行绑定前发现调用方传入的 context "
