...
{
    return "deepcopy_sg 校验 SGE 无效导致复制流程拒绝继续执行";
}

std::string UrmaFailure290::GetRootCauseDesc() const
{
    return "deepcopy_sg 在执行复制前发现调用方传入的 SGE 不满足当前操作要求，通常是对象为空、状态不匹配或与 provider "
...
{
    return "在 URMA_LOG_PATH 中匹配关键日志：Invalid num_sge";
}
