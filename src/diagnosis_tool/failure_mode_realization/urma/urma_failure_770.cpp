...
bool UrmaFailure770::IsValid()
{
    std::string grepOutput = urma_log_helper::RunCommand(
        ...
        "grep -F 'urma_delete_jfs' \"$URMA_LOG_PATH\" 2>/dev/null | "
...
{
    return "urma_delete_jfs 校验 context 无效导致删除流程拒绝继续执行";
}

std::string UrmaFailure770::GetRootCauseDesc() const
{
    return "urma_delete_jfs 在执行删除前发现调用方传入的 context 不满足当前操作要求，通常是对象为空、状态不匹配或与 "
