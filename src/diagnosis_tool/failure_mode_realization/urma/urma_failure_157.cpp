...
bool UrmaFailure157::IsValid(const std::vector<std::string> &fields)
{
    const std::string &message = fields[7];
    return message.find("bondp_create_vcontext") != std::string::npos &&
           message.find("Failed to create p_vjetty_id_table") != std::string::npos;
}
...
{
    return "通过 URMA_LOG_PATH 日志匹配关键字：bondp_create_vcontext，Failed to create p_vjetty_id_table。";
}
