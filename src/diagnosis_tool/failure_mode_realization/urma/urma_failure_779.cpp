...
bool UrmaFailure779::IsValid()
{
    std::string grepOutput = urma_log_helper::RunCommand(
        R"(test -n "$URMA_LOG_PATH" && grep -F 'urma_delete_jfr' "$URMA_LOG_PATH" 2>/dev/null | grep -F 'jfr is deactived, can not delete')");
...
{
    return "urma_delete_jfr 执行删除 JFR 失败导致当前资源状态无法推进";
}

std::string UrmaFailure779::GetRootCauseDesc() const
{
    return "urma_delete_jfr 调用下层 provider、bond 组件或系统接口处理 JFR 时返回失败，当前分支携带 ret/errno "
...
{
    return "在 URMA_LOG_PATH 中匹配关键日志：jfr is deactived, can not delete";
}
