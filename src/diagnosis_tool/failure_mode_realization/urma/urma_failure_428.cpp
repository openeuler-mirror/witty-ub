...
bool UrmaFailure428::IsValid()
{
    std::string grepOutput = urma_log_helper::RunCommand(
        R"(test -n "$URMA_LOG_PATH" && grep -F 'urma_create_jetty' "$URMA_LOG_PATH" 2>/dev/null | grep -F 'Invalid parameter')");
...
{
    return "urma_create_jetty 校验 context 无效导致创建流程拒绝继续执行";
}

std::string UrmaFailure428::GetRootCauseDesc() const
{
    return "urma_create_jetty 在执行创建前发现调用方传入的 context 不满足当前操作要求，通常是对象为空、状态不匹配或与 "
