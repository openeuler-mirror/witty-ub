...
bool UrmaFailure375::IsValid()
{
    std::string grepOutput = urma_log_helper::RunCommand(
        R"(test -n "$URMA_LOG_PATH" && grep -F 'urma_set_jfc_opt' "$URMA_LOG_PATH" 2>/dev/null | grep -F 'Invalid parameter')");
...
{
    return "urma_set_jfc_opt 校验 context 无效导致设置流程拒绝继续执行";
}

std::string UrmaFailure375::GetRootCauseDesc() const
{
    return "urma_set_jfc_opt 在执行设置前发现调用方传入的 context 不满足当前操作要求，通常是对象为空、状态不匹配或与 "
