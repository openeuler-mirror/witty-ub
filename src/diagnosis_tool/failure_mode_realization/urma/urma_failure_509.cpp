...
bool UrmaFailure509::IsValid()
{
    std::string grepOutput = urma_log_helper::RunCommand(
        R"(test -n "$URMA_LOG_PATH" && grep -F 'urma_cmd_query_jetty' "$URMA_LOG_PATH" 2>/dev/null | grep -F 'Invalid parameter')");
...
{
    return "urma_cmd_query_jetty 校验 Jetty 无效导致查询流程拒绝继续执行";
}

std::string UrmaFailure509::GetRootCauseDesc() const
{
    return "urma_cmd_query_jetty 在执行查询前发现调用方传入的 Jetty 不满足当前操作要求，通常是对象为空、状态不匹配或与 "
