...
bool UrmaFailure202::IsValid()
{
    std::string grepOutput = urma_log_helper::RunCommand(
        R"(test -n "$URMA_LOG_PATH" && grep -F 'bondp_import_jfr' "$URMA_LOG_PATH" 2>/dev/null | grep -F 'Invalid param ctx')");
...
{
    return "bondp_import_jfr 校验 context 无效导致导入流程拒绝继续执行";
}

std::string UrmaFailure202::GetRootCauseDesc() const
{
    return "bondp_import_jfr 在执行导入前发现调用方传入的 context 不满足当前操作要求，通常是对象为空、状态不匹配或与 "
