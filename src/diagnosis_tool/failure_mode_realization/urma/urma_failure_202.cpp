...
bool UrmaFailure202::IsValid(const std::vector<std::string> &fields)
{
    const std::string &message = fields[7];
    return message.find("urma_create_jetty_check_trans_mode") != std::string::npos &&
           message.find("jfr cfg is null or trans_mode or order_type invalid with non shared jfr flag.") !=
               std::string::npos;
}
...
{
    return "通过 URMA_LOG_PATH 日志匹配关键字：urma_create_jetty_check_trans_mode，jfr cfg is null or trans_mode or "
           "order_typ"
           "e invalid with non shared jfr flag.。";
}
