...
bool UrmaFailure025::IsValid()
{
    std::string grepOutput = urma_log_helper::RunCommand(
        "test -n \"$URMA_LOG_PATH\" && grep -F 'bondp_uninit' \"$URMA_LOG_PATH\" 2>/dev/null | "
        "grep -F 'Failed to delete global context.'");
...
{
    return "通过 URMA 日志关键字校验：bondp_uninit，Failed to delete global context.";
}
