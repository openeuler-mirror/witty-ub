...
bool UrmaFailure636::IsValid()
{
    std::string grepOutput = urma_log_helper::RunCommand(
        R"(test -n "$URMA_LOG_PATH" && grep -F 'post_send_check_jfs_wr_valid' "$URMA_LOG_PATH" 2>/dev/null | grep -F 'when set cas_wr, either src or dst is NULL')");
...
{
    return "在 URMA_LOG_PATH 中匹配关键日志：when set cas_wr, either src or dst is NULL";
}
