...
bool UrmaFailure703::IsValid()
{
    std::string grepOutput = urma_log_helper::RunCommand(
        "test -n \"$URMA_LOG_PATH\" && grep -F 'urma_ioctl_wait_jfc' \"$URMA_LOG_PATH\" 2>/dev/null | "
        "grep -F 'wait jfc ioctl failed, ret:' | grep -F ', errno:'");
...
{
    return "通过 URMA 日志关键字校验：urma_ioctl_wait_jfc，wait jfc ioctl failed, ret:，, errno:";
}
