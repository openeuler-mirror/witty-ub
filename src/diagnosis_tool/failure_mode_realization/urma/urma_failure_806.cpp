...
bool UrmaFailure806::IsValid()
{
    std::string grepOutput = urma_log_helper::RunCommand(
        R"(test -n "$URMA_LOG_PATH" && grep -F 'urma_delete_jetty_grp' "$URMA_LOG_PATH" 2>/dev/null | grep -F 'Invalid parameter')");
...
{
    return "urma_delete_jetty_grp 校验 context 无效导致删除流程拒绝继续执行";
}

std::string UrmaFailure806::GetRootCauseDesc() const
{
    return "urma_delete_jetty_grp 在执行删除前发现调用方传入的 context "
