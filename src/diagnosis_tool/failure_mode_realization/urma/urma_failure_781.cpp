...
bool UrmaFailure781::IsValid()
{
    std::string grepOutput = urma_log_helper::RunCommand(
        R"(test -n "$URMA_LOG_PATH" && grep -F 'urma_delete_jfr' "$URMA_LOG_PATH" 2>/dev/null | grep -F '[DRV_ERR]Failed to delete jfr, dev_name:' | grep -F ', eid_idx:' | grep -F ', id:' | grep -F ', status:')");
...
{
    return "urma_delete_jfr 执行删除 设备 失败导致当前资源状态无法推进";
}

std::string UrmaFailure781::GetRootCauseDesc() const
{
    return "urma_delete_jfr 调用下层 provider、bond 组件或系统接口处理 设备 时返回失败，当前分支携带 ret/errno "
...
{
    return "在 URMA_LOG_PATH 中匹配关键日志：[DRV_ERR]Failed to delete jfr, dev_name: , eid_idx: , id: , status";
}
