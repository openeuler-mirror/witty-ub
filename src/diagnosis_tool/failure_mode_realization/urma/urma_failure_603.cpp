...
bool UrmaFailure603::IsValid()
{
    std::string grepOutput = urma_log_helper::RunCommand(
        "test -n \"$URMA_LOG_PATH\" && grep -F 'bondp_delete_vcontext' \"$URMA_LOG_PATH\" 2>/dev/null | "
...
{
    return "通过 URMA 日志关键字校验：bondp_delete_vcontext，Failed to urma_cmd_delete_context";
}
