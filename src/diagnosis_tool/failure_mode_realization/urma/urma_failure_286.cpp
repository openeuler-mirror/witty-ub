...
{
    return "create_topo_map 执行创建 拓扑信息 失败导致当前资源状态无法推进";
}

std::string UrmaFailure286::GetRootCauseDesc() const
{
    return "create_topo_map 调用下层 provider、bond 组件或系统接口处理 拓扑信息 时返回失败，当前分支携带 ret/errno "
...
{
    return "在 URMA_LOG_PATH 中匹配关键日志：topo info doesn't have cur_node";
}
