...
#define MODULE_NAME "WITTY-UB-DIAG-TOOL"
...
{
    g_rackContext.RegisterModule<DiagnosisToolModule>();
}

void InitDependencies()
{
    g_rackContext.AddModuleDependencies<DiagnosisToolModule>();
}
...
void CreateModules()
{
    ...
    for (auto type : sortedModules) {
        ...
        if (type == typeid(DiagnosisToolModule)) {
            g_rackContext.InitModule<DiagnosisToolModule>(RackModule::CreateModule<DiagnosisToolModule>());
        } else {
...
int main(int argc, char *argv[])
{
    ...
    try {
        ...
        LOG_WARN << "CreateWittyDir failed, continuing without witty directory";
    }
