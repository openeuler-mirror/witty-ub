...
#define MODULE_NAME "UTIL_VIEW_VISUALIZER"
...
using namespace view_visualizer;

UbseContext &g_rackContext = UbseContext::GetInstance();

void RegisterModules()
{
    g_rackContext.RegisterModule<ViewVisualizerModule>();
}

void InitDependencies()
{
    g_rackContext.AddModuleDependencies<ViewVisualizerModule>();
}
...
void CreateModules()
{
    ...
    for (auto type : sortedModules) {
        ...
        if (type == typeid(ViewVisualizerModule)) {
            g_rackContext.InitModule<ViewVisualizerModule>(RackModule::CreateModule<ViewVisualizerModule>());
        } else {
