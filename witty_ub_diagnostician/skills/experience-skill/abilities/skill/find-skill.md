...
当用户需要查找已有的 Skill 资源时触发，包括但不限于：

- "查找 / 搜索 XX 相关的 Skill"
- "有没有 XX 技能的文档"
- 在执行任务前检索可复用的工作流程
- create-skill / merge-skill / optimize-skill 流程中的前置查重步骤

## 检索能力概述

本能力结合 **SQLite 关键字检索** 与 **FTS5 全文检索**，实现双重匹配：

- **关键字检索**：通过 `keyword_table` 精确匹配 Skill 标签，适合按类别、领域筛选。
...
- `--fields`：指定必须匹配的关键字标签（对应 keyword_table 中的 name 字段）。只有 keywords 包含指定标签的 Skill 才会被召回。
...
- `--experience-ids`：限定仅在指定的 Skill ID 列表中搜索。
...
- `--banned-ids`：排除指定 ID 的 Skill，常用于去重场景。
...
- `--is-hot true`：仅搜索热门 Skill（被频繁检索使用过的 Skill）。
...
- **create-skill**：创建前调用 find-skill 查重。
- **merge-skill**：合并前调用 find-skill 定位相似 Skill。
- **optimize-skill**：优化前调用 find-skill 定位待优化的 Skill。
