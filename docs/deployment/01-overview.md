...
Witty-UB 是一个用于超节点故障定位的工具。**前后端默认支持分离部署**：后端跑在有数据的机器上（无出网要求），前端与 AI Agent 跑在能访问大模型服务的机器上：

```
┌─ 前端节点（能访问 Agent / LLM 的机器） ──────┐      ┌─ 后端节点（有数据的机器） ────────────┐
│                                              │      │                                      │
│  ┌──────────┐                                │      │  ┌───────────┐   ┌────────────────┐ │
│  │  Nginx   │ ── /(api) ──────────────────────────▶  │  │  FastAPI   │──▶│  C++ 工具链     │ │
│  │ (8080)   │    静态资源 dist/                │      │  │ (9772)    │   │ log/topo/diag │ │
│  └────┬─────┘                                │      │  └─────┬─────┘   └────────────────┘ │
│       │ /agent-api/                          │      │        │                              │
│       ▼                                      │      │        ▼                              │
│  ┌──────────────┐                            │      │  ┌──────────────┐                     │
│  │   OpenCode   │                            │      │  │  PostgreSQL  │                     │
│  │  (AI Agent)  │── curl 只读 API ────────────────▶  │  │   (数据存储)  │                     │
│  │   (4096)     │                            │      │  └──────────────┘                     │
│  └──────────────┘                            │      │                                        │
│  LLM key / 经验库（本机）                      │      │  数据卷 /var/witty-ub（仅暴露 9772）      │
└──────────────────────────────────────────────┘      └──────────────────────────────────────┘
```

前端节点与后端节点**可以合并为同一台机器**（单机 All-in-One，`WITTY_ROLE=all`，行为等价于上图两个节点叠加）；Docker、RPM、源码三种部署方式均通过 `WITTY_ROLE` / `WITTY_BACKEND_URL` 等环境变量支持两种形态。
...
| **OpenCode** | AI Agent 服务，提供 AI 辅助故障诊断 | 前端节点（需出网访问 LLM） |
...
| 宿主机脚本 | 5173（前端）、9772（后端） | 15432（PostgreSQL） |
| RPM | 8080（Nginx）、9772（FastAPI） | 15432（PostgreSQL） |
...
- **出网**：能访问大模型服务（OpenCode 调用 LLM API）
- **Node.js 18+** 与 `npm i -g opencode-ai`（源码/RPM 方式）
